...
type WebhookUseCase interface {
	...
	CreateSeparateWebhook(userInfo *api.UserInfo, newWebhook api.WebhookInfo) (api.WebhookInfo, error)
	Exists(ids string) (*api.WebhookInfo, error)
	Update(u *api.UserInfo, scenarioId uint, nodeId uint, nw api.NewServiceNode) error
	UpdateSeparateWebhook(u *api.UserInfo, nodeId uint, nw api.WebhookInfo) error
